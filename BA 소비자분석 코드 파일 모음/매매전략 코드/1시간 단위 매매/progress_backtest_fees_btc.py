...
# BTC(비트코인)에 대한 3년치 60분 시세캔들 ohlcv(당일 시가, 고가, 저가, 종가, 거래량)를 불러옴
# open: 시가, high: 고가, low: 저가, close: 종가, volume: 거래량, value: 해당 시점에 거래된 가격
df = pyupbit.get_ohlcv("KRW-BTC", "minute60", count =26280) 
...
# 앞의 모든  결과를 'BTC_mock_inv'라는 이름의 엑셀 파일로 나타나게 함
df.to_excel("BTC_mock_inv.xlsx") 
